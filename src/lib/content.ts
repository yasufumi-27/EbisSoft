...
export const capabilities: Capability[] = [
  ...
  },
  {
    ...
    demoNote:
      "このデモはブラウザの中だけで完結する検索型のボットです（本サイトは静的配信のため、外部AIへの通信は行いません）。約2時間という実装時間には、この検索エンジン部分の実装も含まれています。当サイトのFAQ・サービス情報を知識源に、日本語の文字N-gramをBM25でスコアリングし、最も近い情報を根拠つきで返しています。単純なキーワード一致（grep）ではなく、語の希少性と文書長を加味して順位づけするため、「いくら？」のような自然文でも料金の項目に当てられます。実案件では、この検索の後段に Claude などの大規模言語モデルを接続して自然文を生成し、必要に応じてベクトル検索を併用したハイブリッド構成にします。「根拠を示す」「知らないことは答えない」という設計思想はデモと同じです。",
    howToUse: [
