...
export const capabilities: Capability[] = [
  ...
  },
  {
    ...
    buildTime: "約15分",
    bullets: [
      "アプリ不要でスマホのカメラにそのまま表示（WebXR）",
      "実物大での配置・回転・移動",
      "人物シルエットとの比較でサイズ感を可視化",
      "iOSはAR Quick Look（USDZ）で同等の体験を提供",
      "既存の3Dモデル資産をそのまま流用",
    ],
    tech: ["WebXR Device API", "hit-test", "Three.js", "glTF / USDZ", "AR Quick Look"],
    useCases: [
      "家具・インテリア・家電のEC",
      "什器・産業機器を提案する法人営業",
      "リフォーム・住宅設備の提案",
    ...
    demoNote:
      "対応端末（AndroidのChrome等）では「ARで表示」ボタンから実際のカメラ映像に重ねられます。非対応の端末・PCでは、部屋を模した空間に実寸で配置するプレビューに自動で切り替わります（こちらもリアルタイム3D描画です）。実案件では、お客様の製品モデルを読み込み、iOS向けにはUSDZを書き出してAR Quick Lookに対応させます。",
    howToUse: [
      "商品を切り替えると、実寸（mm）とともに表示が変わります",
      "スライダーで距離を変えると、人物シルエットと比較できます",
      "対応端末では「ARで表示」からカメラに重ねられます",
    ],
  },
  {
    slug: "voice",
    ...
    demoNote:
      "このデモはブラウザ標準の Web Speech API を使い、実際にマイクからの音声を認識して読み上げています（Chrome / Edge 推奨。Safari・Firefoxでは音声認識が使えないため、テキスト入力で同じ流れを試せます）。回答は当サイトのFAQを知識源にした検索結果です。実案件では、音声をそのまま扱えるリアルタイムAPIに置き換え、より自然な会話にします。",
    howToUse: [
